...
def build_test_set(
    ...
    """
    Extract new interactions in test_G that are not present in train_G.
    Returns {user: set_of_relevant_products}.
 
    mode "all"   -> every new interaction is relevant
    mode "rated" -> only new interactions with rating >= 4 are relevant
    """
    ...
    for u, v, data in test_G.edges(data=True):
        # identify user and product sides
        user    = u if u in train_users else v
        product = v if u in train_users else u
 
        # skip users not in training graph (cold start)
        if user not in train_users:
            continue
 
        # skip interactions already in training graph
        if (u, v) in train_edges or (v, u) in train_edges:
            continue
 
        # apply rating filter if mode is "rated"
        if mode == "rated":
...
def sample_users(
    ...
    """
    Sample n users from the test set using a fixed random seed.
    If test set has fewer than n users, return all of them.
    """
    random.seed(seed)
...
def evaluate_model(
    ...
    for user, relevant in test_set.items():
        ...
        result["diversity_score"] = diversity_score(
            recs, item_users, item_popularity, n_users
        )
 
        user_results.append(result)
 
    # aggregate relevance and diversity across users
    ...
    # centralization metrics (only for path-based models)
...
def run_temporal_evaluation(
    ...
    Args:
        ...
        n_users:      number of users to sample per year
        random_seed:  seed for reproducible user sampling
        eval_modes:   list of evaluation modes ("all", "rated")
        years:        list of training years to evaluate
    """
